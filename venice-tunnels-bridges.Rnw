...
<<statphys.edgebw.density, echo=FALSE, message=F, warning=F, fig.height=4, fig.pos="h!tbp", fig.cap="Edge betweenness density graph for sotoportegos, bridges and everything else; colors as usual.">>=
...
To visualize the distribution of edge betweenness values we have made a density chart in Figure \ref{fig:statphys.edgebw.density}. Density for bridges is quite different from the others: the peak density is at a higher betweenness. There is a peaklet at a lower density that matches the peaks for the other types of edges. This number corresponds to the number of blind alleys in the city (of which there are quite a few). That is not so important, but the fact is that sotoportegos more or less match the rest of the streets, although their betweenness is lower; it also has a minimal peak at a very low value.

<<statphys.edgebw, echo=FALSE, message=F, warning=F, fig.height=4, fig.pos="h", fig.cap="Overall edge betweenness centrality values for bridges, sotoportegos and everything else.">>=
...
ggplot() +
  geom_line(data = bridges_bw_df, aes(x = rank, y = betweenness), color = "red", linewidth = 1) +
  geom_line(data = sotoportegos_bw_df, aes(x = rank, y = betweenness), color = "blue", linewidth = 1) +
  geom_line(data = everything_else_bw_df, aes(x = rank, y = betweenness), color = "yellow", linewidth = 1) +
...
A log-log rank plot of betweenness is shown in Figure \ref{fig:statphys.edgebw}. 
<<statphys.edgebw.top20, echo=FALSE, message=F, warning=F, fig.height=4, fig.pos="h", fig.cap="Top 20 edge betweenness centrality values for bridges, sotoportegos and everything else.">>=
