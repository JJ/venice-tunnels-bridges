...
<<statphys.graph.edge.bw, echo=FALSE, message=F, warning=F, fig.height=6, fig.pos="h!tb", fig.cap="Venice street graph. Colors as above, edge width is proportional to its betweenness. MDS layout has been used in this case.">>=
